...
public static class BiomeRegistryLoader
{
    ...
    /// <summary>
    /// Enforces the surface/underground pairing of spec §6: a surface biome must
    /// name a variant, and any named variant must exist and must itself sit in
    /// the underground layer. The existence and layer checks apply to any biome
    /// that declares a variant, not just surface ones — a deep or underground
    /// biome pointing at a surface biome is the same mistake.
    ///
    /// <para>The "surface biomes must declare one" half is why a missing variant
    /// is fatal rather than skipped (VOID-048): the underground layer is
    /// generated by following the surface column directly above it, so a surface
    /// biome with no variant has no answer for what lies beneath it. Left to
    /// generation that is either a crash mid-world or a silent fallback to some
    /// arbitrary biome, and both are worse than refusing to boot.</para>
    /// </summary>
    private static void CheckUndergroundVariant(BiomeDefinition biome, Registry<BiomeDefinition> biomes)
    {
        if (biome.UndergroundVariant is null)
        {
            ...
            {
                throw new ContentLoadException(
                    $"Surface biome '{biome.Id}' declares no underground_variant. The underground "
                    + "layer follows the surface column directly above it, so every surface biome "
                    + "must name the biome that sits beneath it.");
            }
