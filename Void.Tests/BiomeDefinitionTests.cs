...
public class BiomeDefinitionTests : IDisposable
{
    ...
    /// <summary>
    /// A surface biome that declares NO underground_variant is fatal (VOID-048
    /// AC-4). Before that ticket the null case returned early and loaded clean,
    /// which held together only by luck: every shipped surface biome happened to
    /// declare one. The underground layer is generated by following the surface
    /// column directly above it, so a surface biome without a variant leaves the
    /// generator with no answer for what lies beneath — a crash mid-world, or a
    /// silent fallback to whichever biome sorts first. Boot must refuse instead.
    ///
    /// <para>Underground biomes legitimately have a null variant; they are the
    /// pairing target, never the source. That asymmetry is the whole rule, so
    /// this asserts the surface case is fatal and the underground case is not.</para>
    /// </summary>
    [Fact]
    public void SurfaceBiomeWithNoUndergroundVariantIsFatal()
    {
        WriteFile("biomes.json", $"[{SurfaceJson(undergroundVariant: null)}]");

        ContentLoadException ex = Assert.Throws<ContentLoadException>(LoadTempBiomes);

        Assert.Contains("test:field", ex.Message, StringComparison.Ordinal);
        Assert.Contains("underground_variant", ex.Message, StringComparison.Ordinal);
    }

    /// <summary>
    /// The other half of the asymmetry above: an underground biome with a null
    /// variant must still load. Getting this wrong would make every underground
    /// biome in the shipped roster fatal, so the rule has to know which way it
    /// points.
    /// </summary>
    [Fact]
    public void UndergroundBiomeWithNoUndergroundVariantLoadsFine()
    {
        WriteFile("biomes.json", $"[{UndergroundJson}]");

        Assert.True(LoadTempBiomes().Contains("test:hollows"));
    }
    ...
    public void DeferredReferenceValidationIsFatalOnDanglingRefs()
    {
        ...
        WriteFile("biomes.json", $$"""
            [{
              ...
              "underground_variant": "test:grove_deep"
            },
            {
              // Paired underground biome. Exists only because a surface biome
              // with no underground_variant is fatal since VOID-048, and this
              // fixture loads on its own, so it cannot borrow a shipped one.
              "id": "test:grove_deep",
              "display_name": "Grove Deep",
              "layer_category": "underground",
              "palette": {
                "surface_block": "void:dirt",
                "subsurface_block": "void:dirt",
              ...
              "vegetation": { "trees": [], "plants": [], "decorations": [] },
              "underground_variant": null
            }]
            """);
